...
//     Age  int
// }
// fmt.Sprintf("%+v", Person{"Alice", 30}) // 输出："{Name:Alice Age:30}"

// %#v 输出 Go 语言的语法表示形式（类似代码）
// fmt.Sprintf("%#v", Person{"Alice", 30}) // 输出："main.Person{Name:"Alice", Age:30}"
...
// %q 格式化为带引号的字符串，并对特殊字符进行转义
// fmt.Sprintf("%q", "Hello\nWorld") // 输出："Hello\nWorld"
// fmt.Sprintf("%q", "This is a\tTab") // 输出："This is a\tTab"

// %n 格式化为输出当前已写入的字符数
// fmt.Sprintf("%n", &myWriter) // 输出: 字符数（用于记录写入的字节数）
